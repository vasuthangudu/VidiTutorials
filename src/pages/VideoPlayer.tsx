  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
      ...
      <div className="lg:col-span-2 space-y-6">
        {/* Video Player */}
        <div className="relative aspect-video bg-black rounded-2xl overflow-hidden shadow-2xl ring-1 ring-gray-900/10 group">
          <video
            key={currentUrl}
            src={currentUrl}
            controls
          ...
          <div className="absolute top-4 right-4 z-10">
            <div className="relative">
              <button
                ...
                className="bg-black/50 backdrop-blur-md text-white p-2 rounded-full hover:bg-black/70 transition-all border border-white/20"
              ...
              <AnimatePresence>
                {showQualityMenu && (
                  <motion.div
                    ...
                    className="absolute right-0 mt-2 w-40 bg-white rounded-xl shadow-xl border border-gray-100 overflow-hidden py-1 z-20"
                  >
                    ...
                    {[
                      ...
                      <button
                        ...
                        className="w-full px-4 py-2 text-sm text-left hover:bg-indigo-50 flex items-center justify-between group/item"
                      >
                        <span className={`${selectedQuality === q.label ? 'text-indigo-600 font-bold' : 'text-gray-600'}`}>
                          {q.label}
                        </span>
        ...
        <div className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100">
          <div className="flex flex-wrap items-center gap-2 mb-4">
            <span className="bg-indigo-100 text-indigo-700 text-[10px] font-bold px-2.5 py-1 rounded-full uppercase tracking-wider">
              {video.category}
            </span>
            {video.tags?.map(tag => (
              <span key={tag} className="bg-gray-100 text-gray-600 text-[10px] font-bold px-2.5 py-1 rounded-full uppercase tracking-wider">
                #{tag}
              </span>
            ))}
          </div>
          ...
          <div className="flex flex-wrap items-center justify-between gap-6 py-6 border-y border-gray-50 mb-6">
            <div className="flex items-center gap-4">
              <div className="bg-indigo-600 p-2.5 rounded-xl">
                <User className="w-6 h-6 text-white" />
              </div>
              <div>
                <p className="font-bold text-gray-900">{video.uploaderName || 'Anonymous'}</p>
              ...
              {(isAdmin || video.uploaderUid === user?.uid) && (
                <>
                  <button
                    ...
                    className="flex items-center gap-2 px-5 py-2.5 rounded-xl font-bold bg-amber-50 text-amber-700 hover:bg-amber-100 border border-amber-200 transition-all shadow-sm"
                  >
